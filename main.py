...
def get_turtle_signal(ticker, ticker_data, vix_value, exchange_rate, dynamic_adx_threshold, dynamic_atr_upper_limit, last_buy_price=None, units=0):
    ...
    try:
        ...
        if units > 0 and last_buy_price is not None:
            stop_price = last_buy_price - (2 * last_atr)
        ...
        indicators = {
            ...
            "손절가_usd": stop_price, "목표가_usd": target_price
        }

        is_above_ma200 = last_close > last_ma200

        if units > 0 and units < MAX_UNITS and last_buy_price is not None:
            pyramid_condition = last_close > last_buy_price + (0.5 * last_atr)
            if pyramid_condition:
                return "PYRAMID_BUY", indicators
...
def backtest_strategy(ticker_data, dynamic_adx_threshold):
    ...
    signals['ATR'] = ta.atr(ticker_data['High'], ticker_data['Low'], ticker_data['Close'], length=20)
...
if __name__ == '__main__':
    ...
    for ticker, price_data in data.items():
        try:
            ...
            elif signal == "PYRAMID_BUY":
                pyramid_list.append({
                    'ticker': ticker, 'close': ind['종가'], 'close_krw': ind['종가_krw'],
                    'atr_ratio': ind['ATR비율'], 'target': ind['목표가_usd'], 'stop': ind['손절가_usd'],
                    'target_krw': ind['목표가'], 'stop_krw': ind['손절가'],
                    'quantity': ind['매수가능수량'], 'sector': sector
                })
            
            elif signal == "SELL":
                sell_list.append({
                    'ticker': ticker, 'close_krw': ind['종가_krw'], 'atr_ratio': ind['ATR비율'],
                    'sector': sector
                })
    ...
    pyramid_list = sorted(pyramid_list, key=lambda x: x['atr_ratio'])
    sell_list = sorted(sell_list, key=lambda x: x['atr_ratio'])
...
ATR 비율 1~3% 양호, 3% 이상 고변동성
    ...
    if pyramid_list:
        report_body += "<h2>🔼 피라미딩(추가 매수) 추천 종목</h2><ul>"
        for s in pyramid_list:
            report_body += f"""
            <li><b>{s['ticker']}</b> ({s['sector']}): 추세 지속, 추가 매수 신호 발생 (종가 ${s['close']:.2f} ({format_krw(s['close_krw'])}),
            ATR비율 {s['atr_ratio']:.2f}%, 목표가 ${s['target']:.2f} ({format_krw(s['target_krw'])}), 손절가 ${s['stop']:.2f} ({format_krw(s['stop_krw'])}))
            → <b>추가 매수 가능 수량: {s['quantity']:,}주</b></li>
            """
        report_body += "</ul><hr><br/>"
    
    if sell_list:
        report_body += "<h2>🔻 매도 추천 종목 (청산)</h2><ul>"
        for s in sell_list:
            report_body += f"""
            <li><b>{s['ticker']}</b> ({s['sector']}): 매도 신호 발생 (종가 {format_krw(s['close_krw'])}, ATR비율 {s['atr_ratio']:.2f}%)</li>
            """
        report_body += "</ul><hr><br/>"
...
제공해주신 로그는 오류(Error)가 아니라 **경고(Warning)** 메시지입니다.

### 경고 메시지의 원인 및 해결
`FutureWarning`과 `SettingWithCopyWarning`은 `pandas` 라이브러리가 코드의 비효율적인 부분을 지적하는 메시지입니다. 현재는 정상 작동하지만, 향후 `pandas` 버전(3.0)에서는 오류가 될 수 있는 문법이라는 뜻입니다.

특히 `signals['Position'].iloc[i] = ...`와 같은 부분은 `signals`라는 DataFrame의 복사본에 값을 할당하고 있어서 발생합니다. `pandas`에서는 원본 DataFrame에 직접 값을 할당하는 `.loc`을 사용하도록 권장합니다.

### Pyramiding(피라미딩) 및 상태 추적 로직 추가
말씀하신 대로 피라미딩(Pyramiding)과 청산 규칙을 적용하고, '무상태' 시스템의 한계를 극복하기 위해 사용자의 포지션을 추적하는 **'상태 기반' 로직**을 추가했습니다.

이 새로운 코드에는 다음 기능이 포함되어 있습니다.

* **포트폴리오 추적**: `positions.csv` 파일을 읽어 사용자의 현재 보유 종목과 매수가를 파악합니다.
* **피라미딩 신호**: 보유 종목의 주가가 직전 매수가격 대비 `0.5 * ATR` 이상 상승하면 **`🔼 피라미딩(추가 매수)`** 신호를 보냅니다.
* **청산 신호**: 보유 종목의 주가가 손절가에 도달하거나 추세가 약해지면 **`🔻 매도 추천`** 신호를 보냅니다.

아래 코드를 복사해서 `main.py` 파일의 내용을 덮어쓰고, 아래에 제시된 `positions.csv` 파일도 같은 경로에 만들어주세요.

---
### 단계 1: `main.py` 파일 수정

아래 전체 코드를 복사하여 덮어쓰세요. `backtest_strategy` 함수 내의 경고 발생 코드도 모두 수정되었습니다.
...
def write_positions_file(positions_df, file_path='positions.csv'):
    """DataFrame을 포지션 파일로 저장합니다."""
...
if __name__ == '__main__':
    ...
    # 📌 시드 금액 설정 (사용자의 실제 시드로 변경)
    # TOTAL_SEED_KRW = 20000000 
    ...
    for ticker, price_data in data.items():
        try:
            ...
            # 📌 보유 종목에 대한 상태 확인
            current_position = positions_df[positions_df['ticker'] == ticker]
            last_buy_price = None
            units = 0
            if not current_position.empty:
                last_buy_price = current_position['buy_price'].iloc[-1]
                units = len(current_position)
            
            signal, ind = get_turtle_signal(ticker, price_data, vix_value, EXCHANGE_RATE_KRW_USD, dynamic_adx_threshold, dynamic_atr_upper_limit, last_buy_price, units)
...
ATR 비율 1~3% 양호, 3% 이상 고변동성
    ...
    send_email(subject, report_body)
    print("✅ 리포트 생성 및 전송 완료!")
제공해주신 로그는 오류(Error)가 아니라 **경고(Warning)** 메시지입니다. `pandas` 라이브러리가 코드의 비효율적인 부분을 지적하는 것으로, 현재는 작동하지만 향후 버전에서는 문제가 될 수 있다는 뜻입니다.

`SettingWithCopyWarning`는 DataFrame의 복사본에 값을 할당하고 있기 때문에 발생합니다. 이를 해결하기 위해 `pandas`에서 권장하는 `.loc` 문법을 사용하여 코드를 수정했습니다.

**백테스팅 함수 내의 모든 할당 코드를 `.loc`으로 변경**하여 경고 메시지가 발생하지 않도록 수정했습니다. 아래의 전체 코드를 복사하여 `main.py` 파일의 내용을 덮어쓰고, 다시 실행해 주세요.
        ...
        for table in tables:
            for col in possible_cols:
                if col in table.columns:
                    ...
                    tickers = [t.strip() for t in tickers if isinstance(t, str) and 1 <= len(t) <= 10 and t != 'nan']
                    tickers = [t.replace('.', '-') for t in tickers]
                    print(f"✅ {index_name} 티커 {len(tickers
