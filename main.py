...
def get_index_tickers(index_name):
    ...
    try:
        # User-Agent 헤더 추가하여 HTTP 403 에러 방지
...
def get_turtle_signal(ticker, ticker_data, vix_value, exchange_rate):
    ...
    try:
        ...
        volume_krw_billion = (last_volume * last_close * exchange_rate) / 1e8  # 억원
        ...
        indicators = {
            ...
            "손절가": stop_price_krw, "목표가": target_price_krw, "매수가능수량": buy_quantity
        }
        ...
        try:
            ...
            if signal == "BUY" and is_a_plus_plus(ind):
                a_plus_plus_list.append({
                    ...
                    'quantity': ind['매수가능수량'], 'volume_ratio': ind['거래량비율']
                })
        except Exception as e:
...
ATR 비율 1~3% 양호, 3% 이상 고변동성
    ...
    if REPORT_TYPE == "morning_plan":
        subject = f"🌅 [계획] 터틀 리포트 (VIX: {vix_value:.1f})"
    else:
        subject = f"🌃 [실시간] 터틀 리포트 (VIX: {vix_value:.1f})"
